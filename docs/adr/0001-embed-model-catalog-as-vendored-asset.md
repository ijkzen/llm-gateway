...
「智能填充」依赖 models.dev 的全量模型元数据（363 条，minified 约 290KB）。可选途径：运行时从 models.dev 拉取、前端打包、后端内嵌。运行时拉取使功能受制于外部可用性与网络环境（本服务面向内网部署），前端打包则让后端的尾段匹配拿不到数据。
...
- 二进制体积增加约 290KB，换取零外部依赖与匹配行为的确定性。
