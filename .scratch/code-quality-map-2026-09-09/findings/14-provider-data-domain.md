...
| 14-06 | P3 | 死码/F5 | DeriveActiveEnum 死码 ×4：provider_template.rs 的 ProtocolType+BillingMode、virtual_model.rs 的 LoadBalancingStrategy+FallbackStrategy 全仓零消费；SettingType 是唯一真用的 |
| 14-07 | P3 | 健壮/F5 | 协议/接口编号裸范围 `0..=3`/`0..=4`/`!=4` 散落 12+ 处（backup 7 处/providers 2/virtual_models 2/provider_models 1 等）未绑常量；同仓 virtual_models.rs:124 已有常量区间正面样板 |
| 14-08 | P3 | 简洁 | 模板 extra 回填五个 host 专属函数近重复（mod.rs:230-335）；通用回填只在首次插入分支调用（:53），更新分支不调——种子给某 host 的 extra 新增键永远到不了既有 provider |
| 14-09 | P3 | 性能/简洁 | models.json（293KB include_str!）被 CATALOG/RAW 两个 OnceLock 各解析一次各持一份（catalog.rs:81/84/106/127），双份常驻可合一 |
| 14-10 | P3 | 逻辑/数据 | 模板 upsert 仅按 name 匹配且无删除路径：种子改名/删除留孤儿行，`/match` 同 host 返回重复模板；「尊重用户修改」的注释理由不成立（模板无编辑入口，只有 POST /match） |
| 14-11 | P3 | 健壮 | `is_krill_host`/`is_sensenova_host` 用 matches!（大小写敏感），其余五个 is_*_host 用 eq_ignore_ascii_case；当前安全仅靠调用方都过了 `host_of`（已 lowercase） |
| 14-12 | P3 | 测试覆盖 | build_models_url 只有 v1/v1beta 用例（refresh.rs:136-167），无 vN/非版本末段——14-01 无回归网；集成刷新测试 base_url 一律 `{mock}/v1` |
| 14-13 | P3 | 口径观察 | `recover_quota` 不清 FailureCounter（availability.rs:182-211）与模块头「恢复清零」措辞歧义（enable_manual:267/recover_probe:328 都清）；行为可辩护（额度停用非失败累积）但无注释无测试钉死 |
| 14-14 | P3 | 简洁/性能 | availability 条件更新样板 ×4（148-177/182-211/215-245/302-336）；`set_items_enabled` 逐行 UPDATE（N 往返，可一次 update_many+is_in）；锁 expect/unwrap 毒化即 panic（临界区极小，观察） |
| 14-15 | P3 | F1/单源 | disabled_reason 字面量仍在 failure_recovery.rs:10/33/235 + persist.rs:246（无 09-08 后新增）；枚举只有 as_str 无 parse/PartialEq，消费方无法回流单源；providers.rs:375-379 已用枚举=正面样板 |
| 14-16 | P3 | 注释微 | settings.rs:211 注释「失败时数据库已是新值」措辞误导（settings.update 返回 () 永不失败，指代不明） |
...
### 14-03 max_consecutive_failures 越界静默（P3，校验）
...
### 14-04 测试 harness 双 AppSettings 实例（P3，测试基建）
...
### 14-08 模板 extra 回填重复 + 更新分支不回填（P3，简洁）
...
### 14-09 models.json 双解析（P3，性能/简洁）
...
### 14-12 refresh URL 测试缺口（P3，测试覆盖）
...
### 14-13 recover_quota 不清失败计数（P3，口径观察）
...
### 14-14 availability 样板与逐行 UPDATE（P3，简洁/性能）
...
### 14-15 F1：disabled_reason 单源无反向接口（P3，F1 定案）
...
### 14-16 settings.rs:211 注释措辞（P3，微）
