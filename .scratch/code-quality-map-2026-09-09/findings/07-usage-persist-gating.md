...
| 编号 | 严重度 | 维度 | 一句话 |
| --- | --- | --- | --- |
| 07-01 | P3【已修复 2026-09-10】 | 逻辑/单飞与负缓存 | 真实抓取入口无跨调用单飞且失败无负缓存：cron 刷新 / LB 第三层 / ?refresh=1 / failure_recheck 四路可并发重复抓同一家；持续故障（尤其 DB 写失败）期 LB 每请求一次真实厂商调用——E7 修的是分类（不静默）非放大 |
| 07-02 | P3【已修复 2026-09-10】 | 简洁/缓存双写面 | cron 刷新只写 DB 缓存，mem 缓存持有旧数据直到自身过期——刷新后 mem 命中反而返回旧一轮数据（同一 fetched_at TTL 口径、数据新旧差一轮刷新）；刷新成功后可顺带回填/失效 mem |
| 07-03 | P3【已修复 2026-09-10】 | 健壮/探活时长 | probe_boundary_providers 顺序执行边界供应商、单家最坏 ~260s（04 票同款超时形态）无总体时限——多家边界时拖垮 5 分钟周期（try_lock 自愈跳轮）；建议并发上限或总时限 |
| 07-04 | P3【已修复 2026-09-10】 | 测试覆盖 | mem_cache 失败/取消路径零测试（waiter 收 None 不悬挂、创建者取消清理的 Cleanup guard 无锁定）；`read_usage_cache_many` 全仓零测试；fetch_and_store 的 Database 错误传播无单测 |
| 07-05 | P3【已修复 2026-09-10】 | 测试覆盖 | probe_boundary_providers 零单测（判定/停用/恢复/跳过分支仅靠集成 4 场景）；?refresh=1 与定时刷新并发路径无测试 |

## 各条证据

### 07-01 抓取入口无跨调用单飞 + 失败无负缓存（P3，逻辑/单飞）【已修复 2026-09-10】
...
### 07-02 cron 刷新后 mem 缓存持有旧数据（P3，缓存双写面）【已修复 2026-09-10】
...
### 07-03 probe_boundary_providers 无总体时长上限（P3，健壮）【已修复 2026-09-10】
...
### 07-04 缓存/单飞失败路径零单测（P3，测试覆盖）【已修复 2026-09-10】
...
### 07-05 probe_boundary_providers 与并发刷新路径零单测（P3，测试覆盖）【已修复 2026-09-10】
...
- **07-01 已修复（收敛单飞 + 失败负缓存）**：`UsageMemCache` 新增失败负缓存（1 分钟窗口，`last_attempt` 时间戳；成功清除、`invalidate` 清除）；`fetch_shared_result_inner` 加 `bypass_negative_cache`（手动 `?refresh=1` 强制重取不被挡）。定时刷新与失败复查两路抓取改经 `mem.fetch_shared_stored`（force=true 真重取）——四路（cron/LB/手动/recheck）同刻命中同一家时只发一次厂商调用；持续故障期不再每请求真抓。
- **07-02 已修复**：定时刷新改经 mem 入口后成功结果顺带回填内存缓存——不再出现「刚刷新完 DB 却是内存旧值被优先命中」；`invalidate` 同步清负缓存避免残留。
- **07-03 已修复**：`probe_boundary_providers` 候选过滤抽为纯函数（`boundary_probe_eligible` / `boundary_probe_applicable`），探活本身改 `JoinSet` + 信号量（上限 4，同 refresh_all_usage 形态）——多家处边界不再把整轮拖过 5 分钟周期。
- **07-04 已修复**：新增 3 例 mem 缓存失败/取消路径单测（失败达等待者不悬挂 + 进负缓存且闭包不再执行、成功/失效清负缓存、创建者被 abort 时 Cleanup guard 通知等待者不悬挂）+ 1 例 `read_usage_cache_many` 批读测试（新鲜命中/过期与缺失不计入/空入参）。
- **07-05 已修复**：边界探活候选过滤矩阵补 2 例纯函数单测（`billing_mode`×`disabled_reason`×`usage_enabled` 八组合；剩余百分比 (0,1) 边界×不可用×远离边界×无窗口数据）。

**实施中发现的真实回归（自纠）**：定时刷新初版误用 `force=false` 会退化成读旧缓存，被 `recheck_exhausted_disables_and_auto_restores` 集成测试当场抓出（「余额恢复后应自动启用」失败），改为 `force=true` 后通过。
