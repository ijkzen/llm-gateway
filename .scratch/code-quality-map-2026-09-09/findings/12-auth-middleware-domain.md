...
| 12-05 | P3 | 逻辑/边界 | logout 不在公开清单：过期会话下 POST logout 被中间件 401，服务端清不掉 cookie（前端 401 跳登录兜底，语义毛刺） |
| 12-06 | P3 | 逻辑/边界 | `/v1/messages` 前缀匹配无边界：`/v1/messagesXYZ` 也获 x-api-key 授权口径与 Anthropic 错误格式（同 key 校验，无安全问题） |
| 12-07 | P3 | 简洁 | auth_middleware 持 State 却走 `AppSettings::process_global` 旁路取 lang；`SESSION_TTL_SECS` 常量在 login_response 被字面量 `7*24*3600` 重复 |
| 12-08 | P3 | 性能微观察 | 每个 /api 请求 session+user 两次点查，可 JOIN 合一（SQLite 本地，量微） |
| 12-09 | P3 | 测试覆盖 | 缺口六类：并发双 init / 过期会话自动删除 / x-api-key 入站鉴权 / cookie 属性断言 / auth 端点英文分支 / logout 无会话 401 行为 |
...
### 12-02 会话 Cookie 无 Secure 属性（P3，安全观察）
...
### 12-04 decrypt_or_passthrough 对解不开的密文透传（P3，口径）
...
### 12-05 logout 需会话、过期会话清不掉 cookie（P3，边界）
...
### 12-07 lang 取值旁路与 TTL 字面量重复（P3，简洁）
