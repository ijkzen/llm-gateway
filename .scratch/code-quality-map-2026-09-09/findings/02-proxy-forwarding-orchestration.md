...
| 02-02 | P3【已修复 2026-09-10】 | 错误分支 | failover 尾部「理论不可达」兜底是死代码，且若被触发会对已落 degraded 行的请求重复落一行失败 |
| 02-03 | P3【已修复 2026-09-10】 | 简洁 | calls.rs 两函数尾段（第 2-4 层头组装 + opencode 注入 + host_of）逐字重复 ~20 行 |
| 02-04 | P3【已登记不改 2026-09-10】 | 性能 | Gemini 成员构建时 inline_remote_images 网络下载图片，failover 每成员重下（无跨尝试缓存） |
| 02-05 | P3【已修复 2026-09-10】 | 性能/健壮 | resolve_usage_map 第二层批量 DB 读失败被 unwrap_or_default 吞掉 → 全部落入真实抓取（无退避） |
| 02-06 | P3【已修复 2026-09-10】 | 日志量 | strategy 0/1 每请求 3 条 info 决策日志（用量明细/排序结果/选路结果），高 QPS 噪音（by-design，供复核） |
| 02-07 | P3【已修复 2026-09-10】 | 口径 | 额度剔除致空候选/空成员请求 503 不落 request 表——额度门控拒绝是排障盲区 |
| 02-08 | P3【已修复（补注释）2026-09-10】 | 口径 | forward_chat_direct 成功经 dispatch_success 清零供应商生产连续失败计数，失败却不计数——与注释「失败不触碰可用性状态机」不对称 |
| 02-09 | P3【归 F10 家族批 2026-09-10】 | i18n | native.rs 错误文案 zh-only 硬编码（:62/76 等，F10 族内，此处仅锚点登记） |
| 02-10 | P3【已修复 2026-09-10】 | 简洁/常量 | route.rs:61 retry_enabled = fallback_strategy == 1 裸数字（entity 已定义 RetryEnabledMembers 常量） |
| 02-11 | P3【已修复 2026-09-10】 | 测试覆盖 | build_native_upstream_call 头组装零单测（tests.rs 只测 chat 侧 build_call_sync）；failover 终态分支行为无单测锁定 |
| 02-12 | P3【已修复 2026-09-10】 | 注释一致性 | usage_rank fallback 从 5h 层全层重扫，0% 窗口按数值判负而非「判平」——与层循环注释微矛盾；真实路径被 retain 预剔除保护，纯函数级不可达，仅注释/语义对齐 |
...
### 02-02 兜底死代码（P3，错误分支）【已修复 2026-09-10】
...
### 02-03 calls.rs 头组装尾段重复（P3，简洁）【已修复 2026-09-10】
...
### 02-04 Gemini 远程图片 failover 重复下载（P3，性能）【已登记不改 2026-09-10】
...
### 02-05 resolve_usage_map DB 层失败静默降级（P3，健壮/性能）【已修复 2026-09-10】
...
### 02-06 strategy 0/1 每请求 3 条 info 决策日志（P3，日志量）【已修复 2026-09-10】
...
### 02-07 额度空候选 503 不落 request 表（P3，口径）【已修复 2026-09-10】
...
### 02-08 直连成功清零生产失败计数（P3，口径）【已修复（补注释）2026-09-10】
...
### 02-09 native 中文文案（P3，i18n）【归 F10 家族批 2026-09-10】
...
### 02-10 裸数字 fallback 常量（P3，简洁）【已修复 2026-09-10】
...
### 02-11 测试覆盖缺口（P3，测试）【已修复 2026-09-10】
...
### 02-12 usage_rank fallback 语义注释对齐（P3，纯注释级）【已修复 2026-09-10】
...
## 实施进度（P3 实施批 2026-09-10）

- **02-02 已修复**：删 `last_failure` 状态与尾部兜底，`forward_through_members` 循环后改 `unreachable!()`（契约：空候选提前返回、循环内必出终局），重复落库隐患消失。
- **02-03 已修复**：`calls.rs` 新增 `assemble_outbound_headers`（透传→custom→模板默认→鉴权→opencode 注入五步），`build_upstream_call` / `build_native_upstream_call` 共用；两函数仅保留 URL/body 与协议差异。
- **02-04 已登记不改**（2026-09-10 拍板）：成员多同供应商时该路径罕见，不做 per-request 图片缓存；保留观察。
- **02-05 已修复**：`resolve_usage_map` 第二层 DB 读失败由 `unwrap_or_default` 改为保留内存层结果直接返回 + warn 日志——DB 瞬时故障不再放大为 N 家厂商真实抓取。
- **02-06 已修复**：`lb.rs` 两条决策日志（成员用量明细/排序结果）降 debug，info 仅保留 `route.rs` 选路结果；注释说明深排还原方式。
- **02-07 已修复**：新增 `record_failure_for`（可指占位主体）；`RouteError::NoMembers` 携带 virtual_model_id，`forward.rs`/`native.rs` 两分支与 `failover.rs` 空候选分支均落失败行 + warn 日志（主体键 0/空 model，快照映射不到不产幽灵排行行）。回归测试 `chat_quota_exhausted_returns_503` 补 request 行断言。
- **02-08 已修复（补注释）**：`forward_chat_direct` 文档注释补全成功侧语义（成功=健康信号，经 dispatch_success 清零，拍板保留）。
- **02-09 归 F10 家族批**：proxy/ 域 i18n 消费为零是家族性缺口，单点改无收益；`/v1` 面客户端错误体不站管理面 i18n。
- **02-10 已修复**：`route.rs` 改用 `virtual_model::FallbackStrategy::RetryEnabledMembers as i32`。
- **02-11 已修复**：新增 3 个原生透传头组装单测（URL/model 改写、四层头同源、opencode 会话注入与下游优先）。
- **02-12 已修复**：`cmp_remaining_percent` 文档注释补「0% 在真实路径不可达（subscription_usable 预剔除）」前提说明。
