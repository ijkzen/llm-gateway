...
| 编号 | 严重度 | 维度 | 一句话 |
| --- | --- | --- | --- |
| 09-01 | P3【已修复 2026-09-10】 | 测试健壮 | generator_tests seed_subjects 用两次 `SELECT last_insert_rowid()`（:138-139）取两个 provider 的 id——跨连接读是连接局部的，p1/p2 不保证对应两次 INSERT；现断言恰好对任意 p1≠p2 自洽才绿，池路由变化即脆 |
| 09-02 | P3【已修复 2026-09-10】 | 简洁/nit | `day_level_bucket_skips_percentiles_and_crosses_hours`（generator_tests.rs:551）名实相悖——day 存分位且断言 p50=250（:610-614 注释「天行存分位」），函数名是复制遗留 |
| 09-03 | P3【已修复 2026-09-10】 | 简洁/死开关 | mod.rs 遗留 `#![allow(dead_code)]` 与两处 `#[allow(unused_imports)]` + 陈旧注释（「读路径/任务接入前」「端点接入中」）——全部已接入，allow 掩盖真实死项 |
| 09-04 | P3【已部分修复 2026-09-10】 | 测试覆盖 | tasks 域缺口：heal 只测 hour（day/month/year 补算、未初始化让位、锁忙跳过 Ok(false) 无测）；「两 run 间新闭桶在第二次固化」无直测；时区重算中途失败再入（半删态）无测；finalize 失败→水位不前进→重跑补齐无测 |
| 09-05 | P3【已修复 2026-09-10】 | 测试覆盖 | generator 域缺口：Year 级无直测（与 month 同分支无独立断言）；分位 NULL-entity 排除路径（row_entities pm/ak 缺失）无直测（聚合侧 r5 锁定、分位侧仅空样本间接覆盖）；空 day 桶哨兵数边界无测 |
| 09-06 | P3·观察【观察级保持现状 2026-09-10】 | 逻辑/时区重算窗口 | tasks.rs:101-115 时区重算先 DELETE 全表再逐桶回填——中途失败到下次 run（最长 1h）整段历史走实时兑底（正确但慢）；自愈依赖偏移不匹配持续触发，非永久损坏 |
| 09-07 | P3·观察【已补测试 2026-09-10】 | 逻辑/退避 | tasks.rs:120-137 增量水位：某闭帧 finalize 反复失败（坏行/磁盘）则该级水位永不前移、每次 run 从旧水位重扫失败、无退避——读侧逐桶兑底保正确 |
| 09-08 | P3·记账【已拍板保持现状 2026-09-10】 | 存储/膨胀 | 每桶哨兵 16 行（whole 全 0）× 4 级 × 空桶也写——长期固定开销 ≈ 16 × 桶数（与 request 行量无关），量级记账确认 |

## 各条证据

### 09-01 generator_tests seed_subjects 的 last_insert_rowid 连接亲缘（P3，测试健壮）【已修复 2026-09-10】
...
### 09-02 day 测试函数名与断言相悖（P3，nit）【已修复 2026-09-10】
...
### 09-03 mod.rs 死代码 allow 与陈旧注释（P3，简洁）【已修复 2026-09-10】
...
### 09-04 tasks 域测试缺口（P3，测试覆盖）【已部分修复 2026-09-10】
...
### 09-05 generator 域测试缺口（P3，测试覆盖）【已修复 2026-09-10】
...
### 09-06 时区重算非原子窗口（P3，观察）【观察级保持现状 2026-09-10】
...
### 09-07 增量水位遇反复失败不前移（P3，观察）【已补测试 2026-09-10】
...
### 09-08 空桶哨兵 16 行膨胀记账（P3，存储）【已拍板保持现状 2026-09-10】
...
- **09-01 已修复**：generator 测试的 `seed_subjects` 中 p1/p2 改由 `WHERE name` 反查（不再依赖连接局部的 `last_insert_rowid`），与同函数 api_key/model 主体写法对齐。
- **09-02 已修复**：`day_level_bucket_skips_percentiles_and_crosses_hours` 改名 `day_level_bucket_stores_percentiles_and_crosses_hours`（与断言一致）。
- **09-03 已修复**：删除 `stats_snapshot/mod.rs` 的 `#![allow(dead_code)]` 与四处 `#[allow(unused_imports)]` 及陈旧注释。**暴露并处理了真实死代码**：删 `Coverage::is_empty`、`reader::demote_missing`（哨兵存在性已由帧集合单查取代）、`registry::ALL_ENTITY_TYPES`；把仅测试用的 `HOUR_MS/frames_covering` 收进 `#[cfg(test)]` 导出；分位标量四档常量改为 generator 真实消费（原先用 `_p50` 字符串 replace 推导 `_p90/95/99`，改名会静默漏改）。
- **09-04 已部分修复**：新增 2 条 tasks 测试——自愈覆盖 day/month/year 分支与「未初始化让位」（`heal_covers_day_month_year_and_yields_when_uninitialized`）、水位在整级固化后才前进且回退后可重算（`watermark_only_advances_after_full_level`）。剩余两条（时区重算中途失败再入、finalize 失败注入）缺确定性失败缝，保留后续。
- **09-05 已修复**：新增 2 条 generator 测试——Year 级独立直测（16 行哨兵、不写分位）、分位侧 NULL-entity 排除（pm 已删不产 model/vm_member/api_key_model 分位行，09-09 高估族同源钉死）。
- **09-06 观察级保持现状**：时区重算的 DELETE→回填窗口由读侧兑底兜正确性，换表式重算需磁盘双份不值。
- **09-07 已补测试**：水位不前进的可见语义由上述 `watermark_only_advances_after_full_level` 锁定（失败中止策略本身保持保守现状）。
- **09-08 已拍板保持现状**：空桶 16 行哨兵的年化约 14.6 万行固定开销相对全史请求行量小，压缩需重建等价测试基准，不值。
