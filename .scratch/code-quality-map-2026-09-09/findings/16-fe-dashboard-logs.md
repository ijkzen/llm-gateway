...
| 16-11 | P3 | 简洁 | 三二级页「调用/Token/可靠性」装配重复 ~60 行/页可抽 `AnalysisSections`（overview 统一门控与 model 折线变体保留）——01 盘点候选的定案：抽区块层、不抽整页装配 |
| 16-12 | P3 | 简洁 | dashboard-charts 多余 export 面（CHART_COLORS/OTHER_LABEL/modelLabel/chartColorAt 仅内部使用）；labelInterval 两文件重复；OTHER_LABEL 应走 i18n；**insight 语义色不统一为定案**（按 index 轮转反降可读性） |
| 16-13 | P3 | 契约/健壮 | CSV 多值生成侧无转义：不同供应商同名模型产生重复段（无害）；API Key 名含逗号会被后端 split 错配（呼应 11-16 后端静默吞） |
| 16-14 | P3 | 健壮 | `inferGranularity` 无 year 分支（>24h 全判 month）；当前调用方全显式传 granularity 使 fallback 实不执行，属未爆弹 |
| 16-15 | P3 | 测试观察 | race-period 测试注释漂移（描述 36W 断言 35W）；ISO 周数年界（12/29-1/3 归属相邻 ISO 年）无覆盖 |
| 16-16 | P3 | 性能 | 赛马表全量行渲染无上限/无虚拟滚动（provider-model-rank 大部署可达数百行）；后端 rank 无分页参数，改动需前后端协同 |
| 16-17 | P3 | 测试覆盖 | 缺口族：race-window-control 无专测 / initialWindowFromUrl 真实函数（往返测试是复刻版）/ confirmCustom 非法区间 / 图表纯函数（toRankedModels/inferGranularity）/ buildQuery 序列化零单测 / 列头排序传参（16-04 无回归网）/ 越界分页（16-03）/ 6 个本域 hooks 无直测 |
...
### 16-09 ApiKeyRaceCard 缺 initialWindow（P3）
...
### 16-14 inferGranularity 无 year（P3，健壮）
