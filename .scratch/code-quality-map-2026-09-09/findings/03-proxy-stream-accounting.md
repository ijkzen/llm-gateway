...
| 03-04 | P3【已修复（交叉注释）2026-09-10】 | 简洁/残余分叉 | 事件驱动循环双份：relay 泵内联循环 vs collect_stream_events 同语义独立实现，带内错误处理已在两份分叉（03-01 根因之一） |
| 03-05 | P3【已修复 2026-09-10】 | 健壮/诊断质量 | Anthropic/Gemini 非流式读体失败被 unwrap_or_default 吞成「解析上游响应失败：EOF」——真实原因丢失，文案误导排障 |
| 03-06 | P3【已修复 2026-09-10】 | 测试覆盖 | relay.rs/dispatch.rs 无内联单测：StreamOutcome 组合判定、accumulate_chunks 聚合行为、断开分支均无直接锁定 |
| 03-07 | P3【已修复 2026-09-10】 | 测试覆盖 | 带内错误客户端形状零测试（与 03-01 伴生，修复时补三协议回归） |
| 03-08 | P3【已拍板保持现状 2026-09-10】 | 性能/健壮 | SseSplitter 缓冲无上限（无 `\n` 数据无限累积）——上游数据不可信，可被异常上游无限膨胀内存 |
| 03-09 | P3【已拍板文档化 2026-09-10】 | 口径/过滤边界 | usage-only 尾块过滤只认「空 choices+usage」形态，usage 并入非空终块的上游（DeepSeek 类）在客户端未请求时仍透传 usage |
...
### 03-04 事件驱动循环双份（P3，简洁/残余分叉）【已修复（交叉注释）2026-09-10】
...
### 03-05 AG 非流式读体失败原因被吞（P3，健壮/诊断质量）【已修复 2026-09-10】
...
### 03-06 域内无内联单测（P3，测试覆盖）【已修复 2026-09-10】
...
### 03-07 带内错误零测试（P3，测试覆盖，03-01 伴生）【已修复 2026-09-10】
...
### 03-08 SseSplitter 缓冲无上限（P3，性能/健壮）【已拍板保持现状 2026-09-10】
...
### 03-09 usage-only 尾块过滤形态过窄（P3，口径/过滤边界）【已拍板文档化 2026-09-10】
...
- **03-04 已修复（交叉注释，拍板不抽公共原语）**：`relay_stream` 与 `collect_stream_events` 两处文档注释互相点名，并写明「两份必须同步演化——03-01 假成功 bug 正是整流侧与泵侧分叉的产物」，改动任一份时对照另一份。
- **03-05 已修复**：`dispatch.rs` Anthropic/Gemini 非流式臂把 `read_body(...).unwrap_or_default()` 拆成显式读失败分支（与 OpenAI 直通臂同形），落库/日志原因归位为「读取上游响应失败：…」而非伪装的「解析失败：EOF」。
- **03-06 已修复**：`relay.rs` 新增 5 个 StreamOutcome 单测（干净流成功无原因、三错误源各自判败、多源优先级稳定、断开不翻失败仅补原因、断开与错误并存取上游原因）。
- **03-07 已修复**：新增 Responses（`response.failed`）与 Gemini（`{"error":…}` 且错误前有内容增量）两条带内错误集成回归——断言 error 帧 + [DONE] + 内容增量保留 + 落库失败，与既有 Anthropic 一例构成三协议锁定。
- **03-08 已拍板保持现状**：正常上游行长短（<~100KB），无上限风险仅限异常/恶意上游，实施成本（四处调用点联动 + 上限语义定夺）当前不值。
- **03-09 已拍板文档化**：`is_usage_only_chunk` 文档注释写明已知边界（DeepSeek 类把 usage 并入非空 choices 终块时过滤不命中，属浅泄漏），不扩展判定（需先确证该类上游终块形态）。
