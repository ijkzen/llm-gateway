...
| 18-05 | P3【已修复 2026-09-10】 | 逻辑 | 断线重连错过 run_ended 时 idle 分支不 invalidate runs，历史执行列表长期停留旧快照 |
| 18-06 | P3【已修复 2026-09-10】 | 逻辑/契约 | cron 表达式前端仅 `.min(1)` 非空（语法校验后端独有且仅变更时），非法表达式只得泛化 toast 无字段级错误 |
| 18-07 | P3【已修复 2026-09-10】 | 逻辑 | 立即执行后固定 1s 失效刷新，但 last_run_at 在执行**结束**才回写——超过 1 秒的任务刷新不到新时间 |
| 18-08 | P3【已修复 2026-09-10】 | 逻辑/一致性 | CronJobDeleteDialog 的 onError 也关弹窗，与 Provider/ApiKey/Setting/VirtualModel 删除弹窗「失败保持打开」不一致 |
| 18-09 | P3【已修复 2026-09-10】 | 性能 | 实时日志列表无 memo/虚拟化，每条新日志整表重渲染+逐行重算 formatDateTime，2000 条上限下累计 O(n²)；18-01 的 key 冲突进一步退化 diff |
| 18-10 | P3【已修复 2026-09-10（关键缺口）】 | 测试覆盖 | cron 域缺口族：reset/run_started/onerror/Lagged/seq 去重分支全未测；cron-jobs 页只验选中态（弹窗全被 mock 成桩）；List/Detail/Edit/Delete 四组件零测试；两 hooks 零直测 |
| 18-11 | P3【已修复 2026-09-10】 | 健壮/性能 | SSE onerror 恒置 reconnecting 无退避无终止：会话过期 401 时浏览器无限重连、界面永久「重连中」 |
| 18-12 | P3【已修复 2026-09-10】 | UX | SettingEditDialog 不展示被编辑的 key/type（Json 弹窗有 key），上下文缺失 |
| 18-13 | P3【已修复 2026-09-10】 | 逻辑 | Json 结构化编辑静默丢数据：数组丢空串条目、对象丢空键行、重复键后者覆盖前者（现有测试反而固化丢弃行为） |
| 18-14 | P3【已修复 2026-09-10】 | 状态 | ImportDialog 成功/关闭后 file 状态不复位、file input 未清空：重开显示旧文件且确认可用，可重复误导入 |
| 18-15 | P3【已修复 2026-09-10】 | 契约 | 改密 zod 按 JS 字符串长度、后端按 UTF-8 字节：非 ASCII 密码前端放行后端 400，文案「characters」误导 |
| 18-16 | P3【已修复 2026-09-10（关键缺口）】 | 测试覆盖 | settings 域缺口族：settings.tsx 页/SettingEditDialog/use-settings/not-found 均无测试（01 盘点两项确认成立）；Json raw 模式改动后保存、改密长度边界、导入残留（18-14）无覆盖 |
| 18-17 | P3【已修复 2026-09-10】 | 简洁 | 三弹窗「mutation+toast+onOpenChange」样板同构可抽共享 helper（use-settings 冻结，宜放域内工具） |
| 18-18 | P3【已修复 2026-09-10（结论修正：effect 非冗余）】 | 简洁 | SettingsTable 的 pageIndex 重置 effect 与 TanStack 默认 autoResetPageIndex 重叠且依赖不含 searchQuery/typeFilter——注释所述职责实际由框架承担，冗余 |
| 18-19 | P3【已修复 2026-09-10】 | 观察 | 类型筛选下拉只有 5 个已知类型，后端可能返回的 `Unknown` 行不可筛选（badge 有兜底） |
| 18-20 | P3【已修复 2026-09-10】 | 观察 | 设置删除无前端保护键禁用（language/timezone 照常展示删除菜单，靠后端 400 兜底；11-09 同族） |
...
### 18-05 重连 idle 不刷新历史列表（P3）【已修复 2026-09-10】
...
### 18-08 删除失败关弹窗（P3，一致性）【已修复 2026-09-10】
...
### 18-09 实时日志 O(n²) 渲染（P3，性能）【已修复 2026-09-10】
...
### 18-10 cron 域测试缺口（P3）【已修复 2026-09-10（关键缺口）】
...
### 18-14 ImportDialog 文件残留（P3，状态）【已修复 2026-09-10】
...
### 18-15 改密长度口径（P3，契约）【已修复 2026-09-10】
...
### 18-16 settings 域测试缺口（P3）【已修复 2026-09-10（关键缺口）】
...
- **18-05 已修复**：SSE 的 `snapshot` 与 `idle` 分支一并 `invalidateQueries(runs)`——断线期间结束的执行，重连后历史列表即刷新。
- **18-06 已修复**：`CronJobEditDialog` 的 onError 识别后端「表达式」类错误并落到 `expression` 字段（`form.setError`），不再只给泛化 toast。
- **18-07 已修复**：`useRunCronJob` 由固定 1s 单次失效改为「轮询到 last_run_at 变化或达上限」（2s × 15 次），长任务也能刷到新时间；卸载时清理计时器。
- **18-08 已修复**：`CronJobDeleteDialog` 失败不再关闭弹窗（与其余三个删除弹窗一致）。
- **18-09 已修复**：日志行抽 `memo(LogLine)`，追加新日志时旧行不再重渲染。
- **18-10 已修复（关键缺口）**：新增 SSE 分支回归——`run_started` 清空切换、`reset` 按 seq 合并（18-02 网）、断开进入重连态、连续失败达上限停止并提示（18-11 网）；测试补 `@/lib/api` mock 以驱动 reset 的真实拉取路径。
- **18-11 已修复**：SSE 改为自带退避重连（1s→2s→…→30s，上限 5 次），超限关闭连接并置 `reconnectExhausted`；日志弹窗展示「实时连接已中断」提示 + 刷新按钮（新增 `cronJobs.reconnectFailed` 双语词条），不再永久「重连中」。
- **18-12 已修复**：`SettingEditDialog` 头部补 `DialogDescription` 显示当前编辑的 key 与类型。
- **18-13 已修复**：Json 结构化编辑保存前校验——数组空条目 / 对象空键 / 重复键均拦下并提示（新增三个双语词条），不再静默丢弃；原有「空键行被丢弃」测试改写为断言拦截行为。
- **18-14 已修复**：`ImportDialog` 新增 `clearFile()`（清 state + 清 input.value），导入成功后与弹窗关闭时都调用——重开不再残留旧文件。
- **18-15 已修复**：改密长度改为按 UTF-8 字节校验（`TextEncoder`），文案改「字节」；新增中文密码边界回归（128 汉字被拦、128 ASCII 放行）。
- **18-16 已修复（关键缺口）**：新增设置页整页测试（Json/非 Json 弹窗路由、错误态重试、内置键删除禁用）与 `SettingEditDialog` 专属测试（Int/Float 非法值当场标错不提交、Bool 渲染开关、头部 key+type）。
- **18-17 已修复**：新增域内 helper `useSettingSubmitCallbacks`，三处弹窗的「成功关窗 + toast」样板收敛。
- **18-18 结论修正并修复**：实测 TanStack 的 `autoResetPageIndex` 在「搜索收窄结果」场景并不重置页码（新增回归测试即红），故 effect 不是冗余代码——改为依赖 `filteredSettings` 使其真正生效；原注释所述职责此前并未履行。
- **18-19 已修复**：类型筛选下拉补 `Unknown` 选项（后端无法映射的存储类型）。
- **18-20 已修复**：`SettingsTable` 对内置键（language/timezone）禁用删除菜单项并加 title 提示（新增 `settings.builtinNotDeletable` 双语词条），不再只靠后端 400。
