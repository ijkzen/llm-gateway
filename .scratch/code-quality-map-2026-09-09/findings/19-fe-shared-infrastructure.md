...
| 19-08 | P3 | 逻辑/潜在 | chart.tsx tooltip `item.value &&` 吞 0 值（当前调用全传 formatter 不可达，属潜在缺陷） |
| 19-09 | P3 | 逻辑 | sidebar Ctrl/Cmd+B 无可编辑目标/event.repeat 守卫；useIsMobile 首帧恒 false 移动端闪一帧桌面侧栏 |
| 19-10 | P3 | i18n | error-boundary.tsx:29-30/36 硬编码中文（锚点确认归 20 票；类组件可用全局 i18n 单例直调）；另观察：hasError 不随路由重置，出错后只能整页刷新 |
| 19-11 | P3 | 测试覆盖 | `ui/` 与 `data-table/` 零测试目录；ErrorBoundary/LocaleToggle/DataTablePagination/ChartContainer/useSidebar/getPageNumbers 零测试引用 |
| 19-12 | P3 | 逻辑 | `/login` 懒加载但在 AppLayout 的 Suspense 之外——直达/硬刷新 /login 白屏等 chunk（无 boundary 不抛错只挂起） |
| 19-13 | P3 | 性能 | react-query `retry:1` × ky `retry:1` 叠加 → 失败查询最多 4 次 HTTP 尝试（与 19-02 叠加后 4xx 也打满） |
| 19-14 | P3 | 逻辑 | 全局 401 跳转 `location.assign("/login")` 丢失 RequireAuth 的 `from` 回跳——会话过期后登录回不到原页面 |
| 19-15 | P3 | 测试覆盖 | `lib/api.ts` 零测试：unwrap 两抛错分支/401 钩子三判定/beforeError 三分支/fetchHealth 特例全裸 |
| 19-16 | P3 | 逻辑 | `middleEllipsis` 守卫用 UTF-16 length、截断按码点：代理对文本返回值 .length 可超 maxLength（SVG 轴宽预算面窄） |
| 19-17 | P3 | 测试覆盖 | utils 缺口：getPageNumbers（off-by-one 风险五分支）/formatPercent/formatDateTime/cn/localeOf 无测试 |
| 19-18 | P3 | 模块间 | 时区键 `"timezone"` 三处独立定义（i18n/index、use-stats-time-zone、test/setup）+ 默认值两处；statsFilterKeySegments 重抄 StatsFilter 形状未复用 |
| 19-19 | P3 | 简洁 | `pages.ts` 的 `PAGES` 死代码（全仓只用 NAV_GROUPS）；`ApiError.statusCode` 无消费点（投机面） |
| 19-20 | P3 | i18n | `lib/utils.ts` 的 `formatDateTime` 硬编码 `toLocaleString("zh-CN")`（ProviderDetail/CronJobDetail 消费，EN 界面仍中文格式） |
| 19-21 | P3 | 性能 | vite manualChunks 的 `react-router-dom` 匹配不到 react-router 核心包——router chunk 实为薄壳，分包意图部分落空 |
| 19-22 | P3 | 测试基建 | setup.ts 全局 mock `use-stats-time-zone`：真实 hook（读设置表挑 timezone 行）零覆盖，mock 常量与真实模块双份字面量静默漂移面 |
...
### 19-05 MidEllipsis 强制同步重排（P3，性能）
...
### 19-07 multi-select 硬编码 id（P3，健壮）
...
### 19-22 setup 全局 mock 掩盖真实 hook（P3，测试基建）
