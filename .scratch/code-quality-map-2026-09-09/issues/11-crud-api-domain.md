...
- 拍板二：全局 5MB body 上限**保持现状**（DoS 兜底，接受巨型多模态 /v1 请求 413，真实需求出现再议放宽）。→ **已推翻（2026-09-11）：用户拍板去除该上限**（`DefaultBodyLimit::disable()`），回归测试见 `tests/auth_integration.rs::oversized_request_body_reaches_handler`。
