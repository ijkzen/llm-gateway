...
fn cache_age_fresh(fetched_at: chrono::DateTime<Utc>) -> bool {
...
/// LB 用量感知排序的内存缓存：与数据库缓存同新鲜度口径（fetched_at 10
/// 分钟），命中免一次 DB 往返；缓存缺失时的真实抓取按 provider 单飞去重
/// （同一瞬间多个并发请求只发一次上游调用）。Provider 更新/删除时调用
/// [`UsageMemCache::invalidate`] 保持与数据库缓存同失效语义。
    ...
    pub async fn read_many(&self, provider_ids: &[i32]) -> HashMap<i32, UsageData> {
        ...
        provider_ids
            ...
            .filter_map(|id| {
                ...
                let age = now.signed_duration_since(data.fetched_at);
