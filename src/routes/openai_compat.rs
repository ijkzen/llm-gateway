...
/// OpenAI 格式的错误响应（不走内部 {code,msg,data} 包装）。
fn openai_error(
    status: StatusCode,
...
async fn get_model(State(state): State<AppState>, Path(display_id): Path<String>) -> impl IntoResponse {
    ...
    match virtual_model::Entity::find()
        ...
        .await
    {
        Ok(Some(model)) => (StatusCode::OK, Json(model_object(&model.display_id, model.created_at))),
