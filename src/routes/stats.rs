...
pub fn routes() -> Router<AppState> {
    Router::new()
        ...
        .route("/provider-rank", get(rank::provider_rank))
        .route("/virtual-model-rank", get(rank::virtual_model_rank))
        .route("/provider-model-rank", get(rank::provider_model_rank))
        .route(
            "/virtual-model-member-rank",
            get(rank::virtual_model_member_rank),
        )
        .route("/api-key-rank", get(rank::api_key_rank))
...
pub(crate) use rank::rank_metric_sql;
