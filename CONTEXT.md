...
**取数窗口 (Query Window)**:
查询侧时间窗的传递形态：只把「窗口定义」（周期 + 偏移 / 自定义区间）与设置表时区作为 query key 的稳定身份，绝对起止 `{startTime, endTime}` 延到取数时（`queryFn` 内）按调用时刻解析（ADR-0022）。存在的理由是把「随时间变化的取值」挡在 query key 之外——否则重取（手动刷新/聚焦/重试）复用挂载时刻的旧窗口，当前周期被永久截在上一次解析，刷新看不到新数据。展示用 `now`（窗口控件标题/标签）与被取数窗口相互独立：前者可固化，后者不可。
_Avoid_: 时间窗口（泛称，与用量窗口/快照桶混淆）、窗口参数（指代裸传 startTime/endTime 的旧写法）
