...
- **核心功能**:
  ...
  - `/v1` OpenAI 兼容转发：对外仅提供 `POST /v1/chat/completions`（Bearer API Key 鉴权），按虚拟模型 LB 策略选成员转发到 OpenAI Compatible / OpenAI Responses / Anthropic / Gemini 四种上游协议（转换逻辑参考 nyro 与 LiteLLM），支持流式与非流式、failover 重试（408/429/500/502/503/529）；下游 `user-agent` 按透传 allowlist 原样转发所有上游；对 `opencode.ai` 上游注入 `x-opencode-session` 会话头（OpenCode Go 会话亲和：下游透传/custom_header 值优先，缺失时按 API Key 派生稳定 UUID 回退）。
